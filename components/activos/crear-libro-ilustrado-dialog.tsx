...
interface CrearLibroIlustradoDialogProps {
  onLibroCreado?: () => void;
}

export function CrearLibroIlustradoDialog({ onLibroCreado }: CrearLibroIlustradoDialogProps) {
  const [open, setOpen] = useState(false);
  const [nombre, setNombre] = useState('');
  const [descripcion, setDescripcion] = useState('');
  const [categoria, setCategoria] = useState('LIBRO_ILUSTRADO');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Preparar datos para crear el libro ilustrado
      const libroData = {
        nombre,
        descripcion,
        categoria,
        ...
        licencia: 'COPYRIGHT',
        estado: 'DRAFT',
        etiquetas: ['libro', 'ilustrado', 'infantil'],
      };
      ...
      if (result.success) {
        ...
        setError(result.error || 'Error al crear el libro ilustrado');
      }
    } catch (error: any) {
      console.error('Error al crear libro ilustrado:', error);
      setError(error.message || 'Error al crear el libro ilustrado');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Crear Libro Ilustrado
            </DialogTitle>
            <DialogDescription>
              Crea un nuevo libro ilustrado para niños.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="nombre-libro-ilustrado" className="text-right">
                Nombre
              </Label>
              <div className="col-span-3">
                <Input
                  id="nombre-libro-ilustrado"
                  value={nombre}
                  onChange={(e) => setNombre(e.target.value)}
                  placeholder="Mi Libro Ilustrado"
            ...
            <div className="grid grid-cols-4 items-start gap-4">
              <Label htmlFor="descripcion-libro-ilustrado" className="text-right pt-2">
                Descripción
              </Label>
              <div className="col-span-3">
                <div className="h-40 rounded-md border border-input focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2 overflow-hidden">
                  <ReactQuill
                    ...
                    placeholder="Describe tu libro ilustrado..."
                    className="h-32"
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="categoria-libro-ilustrado" className="text-right">
                Categoría
              </Label>
              <div className="col-span-3">
                <Input
                  id="categoria-libro-ilustrado"
                  value={categoria}
                  onChange={(e) => setCategoria(e.target.value)}
                  placeholder="LIBRO_ILUSTRADO"
                  className="w-full rounded-md border border-input focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
                />
              </div>
            </div>
